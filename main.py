...
for train_index, test_index in kf.split(shiyan_samples):
    if iter < 6:
        ...
        # np.save("./piRNA2/train_samples_{0}.npy".format(iter),train_samples)
        train_samples = np.load("./piRNA/train_samples_{0}.npy".format(iter))  # 读取每一次的数据
        # test_samples = shiyan_samples[test_index, :]  # 测试样本
        # np.save("./piRNA2/test_samples_{0}.npy".format(iter), test_samples)
        ...
        ***************************** 使用疾病的GIP特征构建图提取疾病的GIP特征 ******************************
        ...
        # 读取最好的实验结果
        test_label = np.load("./result/VGAE/VGAE10_test_label_{0}.npy".format(iter))
        y_pred = np.load("./result/VGAE/VGAE10_y_pred_{0}.npy".format(iter))
        auc = roc_auc_score(test_label, y_pred)
        # 得到每一次的测试集的标签值和预测值，然后进行数据可视化（画图，画表格）
        # np.save("./result/GAT/GAT2_test_label_{0}".format(iter), test_label)
        # np.save("./result/GAT/GAT2_y_pred_{0}".format(iter), y_pred)

        # np.save("./result/VGAE/VGAE_test_label_{0}".format(iter), test_label)
        # np.save("./result/VGAE/VGAE_y_pred_{0}".format(iter), y_pred)
